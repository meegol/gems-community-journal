    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--text-muted)] mt-0.5">
              Ranked by Net Realized R-Multiple Return & Consistency
            </p>
          </div>
        </div>
      ...
      {entries.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 rounded-2xl border border-dashed border-[var(--border-color)] bg-[var(--bg-card)]">
          ...
          <div>
            <p className="text-base font-extrabold text-[var(--text-primary)]">No Rankings Yet</p>
            <p className="text-xs text-[var(--text-muted)] mt-1 max-w-xs">
              The leaderboard will populate as members log trades and opt them into the community. Be the first on the board.
            </p>
          </div>
        </div>
      ) : (
                    ...
                    >
                      <td className="py-4 px-4 text-center">
                        {isTop1 ? (
                          ...
                          <div className="mx-auto flex h-7 w-7 items-center justify-center rounded-full bg-slate-300 text-black font-extrabold">2</div>
                        ) : isTop3 ? (
                          <div className="mx-auto flex h-7 w-7 items-center justify-center rounded-full bg-amber-700 text-white font-extrabold">3</div>
                        ) : (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-extrabold text-[var(--text-primary)] flex items-center gap-1.5">
                              ...
                              {entry.isCurrentUser && (
                                <span className="text-[9px] font-extrabold px-1.5 rounded bg-emerald-500 text-black">YOU</span>
                              )}
                            </p>
                      ...
                      <td className="py-4 px-4 text-right font-bold text-emerald-400">{entry.winRate.toFixed(1)}%</td>
                      <td className="py-4 px-4 text-right font-extrabold text-emerald-400">+{entry.totalR.toFixed(1)}R</td>
                      <td className="py-4 px-4 text-right font-extrabold text-[var(--text-primary)]">+${entry.totalPnL.toLocaleString()}</td>
                      <td className="py-4 px-4 text-center font-semibold text-[var(--text-muted)]">{entry.totalTrades}</td>
                    </tr>
                  );
                })}
              </tbody>
